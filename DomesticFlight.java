...
public class DomesticFlight extends JFrame
{
	...
	JLabel LFrom, LTo, LBookingDate, LClass, LAdult, LChildren, LInfant, LBookingDetails, LPassengerDetails, LDate, LImg1, LImg2, LNotes, LNames;
	JTextField TFBookingDate,TNames;
	Icon img1, img2;
	JButton BFindFlight;
	...
	public DomesticFlight(LoginPage type1)
	{
		...
		String[] sItem1={"Mumbai","Patna","Amritsar","Goa","Agra","Gopalganj","Shimla","ShriNagar"};
		String[] sItem2 ={ "Bangalore", "Delhi ", "Patna", "Goa", "Gopalganj", "Amritsar", "Vishakapatnam" };
		String[] sItem3={"Economic"};

		this.type1 = type1;
		PPanel1 = new JPanel(null);
		PPanel1.setPreferredSize(new Dimension(500,200));
		PPanel1.setBackground(Color.decode("#7C96AB"));
		...
		PPanel2.setBackground(Color.decode("#FFD3B0"));

		LPassengerDetails=new JLabel("<html><b><font color=\"#C71585\">PassengerDetails</font></b></html>");

		TNames = new JTextField(20);
		LNames = new JLabel("Names: ");
		...
		LNotes = new JLabel("<html><body><p>NOTE: Business Class Bookings are</p><p> currently under development. Will</p><p>be working soon!</p></body></html>");
		...
		LNames.setBounds(40,225,100,20);
		TNames.setBounds(140,225,100,20);
...
class button3 implements ActionListener
{
	...
	//defining the ActionListener Interface Methods
	public void actionPerformed(ActionEvent e)
	{
		...
		String sBookingDate =(String) type.TFBookingDate.getText();
		String sNames=(String)type.TNames.getText();
		Integer iPrice=0;
		String sTime="";
		...
		{
			try{
				for(i=0;i<type1.row1.length;i++){
					if(type1.row1[i][0].equals(sFrom)){
				...
				if(flg==0){
					JOptionPane.showMessageDialog(null, "No direct Flight Available");
					System.exit(0);
				}

			}catch(Exception e1)
			{
				//Login Error will be catched here. ie NULLPOINTER
				JOptionPane.showMessageDialog(null, "No Valid rights, Access Denied!");
				System.exit(0);
			}
		}
		iPrice = (iPrice*iAdult)+(iPrice*(iChildren/2));

		type.setTitle("Rs "+iPrice + ", Time:" + sTime);
		...
		String[] sTempNames=new String[1250];
		...
		try
		{
			...
			do
			{
				...
				sTempNames[iCount]=save1.sNames;
...
//			JOptionPane.showMessageDialog(null,e);
		...
		else
		{
			...
			{
				new PrintTicket1(sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice, sTime,sNames);
			try
			{
//write into data
				Save2 save2=new Save2(sFrom, sTo, sClass, iAdult, iChildren, iInfant, sBookingDate, iPrice, sTime,sNames);
				ObjectOutputStream OOS1 = new ObjectOutputStream(new FileOutputStream("save2"));
				for(i=0;i<iCount;i++)
				{
					Save2 temp1=new Save2(sTempFrom[i], sTempTo[i], sTempClass[i], iTempAdult[i], iTempChildren[i], iTempInfant[i], sTempBookingDate[i], iTempPrice[i], sTempTime[i],sTempNames[i]);
...
{
	String sFrom, sTo, sClass, sBookingDate, sTime, sNames;
	Integer iPrice, iAdult, iChildren, iInfant;

	public Save2(String sFrom, String sTo, String sClass, Integer iAdult, Integer iChildren, Integer iInfant, String sBookingDate, Integer iPrice, String sTime, String sNames)
	{
		this.sFrom=sFrom;
		this.sTo=sTo;
		this.sClass=sClass;
...
//		this.iCount = iCount;
	...
	{
		return sFrom+" "+sTo+" "+sClass+" "+iAdult+" "+iChildren+" "+iInfant+" "+sBookingDate+" "+iPrice+" "+sTime+" "+sNames;
	}
}